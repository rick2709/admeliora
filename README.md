...
- The full logo lockup used in the header/footer/mobile menu (`components/ui/Logo.tsx`) is the customer's logo artwork with the white background removed, exported as one transparent PNG — `public/images/logo-full.png` (navy/blue). It bakes in the icon, "AD MELIORA", the tagline, and the website/email line, so `Logo` renders it as a single image on its own white card (the customer wants the blue mark on white always, never a white-on-dark version) sized via a `height` prop (width follows the artwork's aspect ratio). Regenerate the PNG if the customer supplies updated logo artwork.
