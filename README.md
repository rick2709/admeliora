...
- The full logo lockup used in the header/footer/mobile menu (`components/ui/Logo.tsx`) is the customer's logo artwork with the white background removed, exported as two transparent PNGs — `public/images/logo-full.png` (navy/blue, for light backgrounds) and `public/images/logo-full-light.png` (white silhouette, for dark backgrounds). It bakes in the icon, "AD MELIORA", the tagline, and the website/email line, so `Logo` renders it as a single image sized via a `height` prop (width follows the artwork's aspect ratio) rather than separate icon + coded text. Regenerate both if the customer supplies updated logo artwork.
